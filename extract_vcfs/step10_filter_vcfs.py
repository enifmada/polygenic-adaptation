...
sys.path.append(str(Path(__file__).resolve().parent.parent/"src"/"polygenic_adaptation"))
...
def main():
    ...
    parser.add_argument("-maf", "--min_allele_freq", type=float, default=0.05,
                        help="filters out replicates with mean minor allele frequency < MAF")
    parser.add_argument("--min_sample_density", type=float, default=0.1,
    ...
    vcf_dates = pd.read_csv(args.input_table, usecols=["Genetic_ID", "Date_mean"], sep="\t").to_numpy()
