...
ANNO_FILE_1240K = pathlib.Path(f"AADR/{AADR_VERSION}_1240K/aadr_{AADR_VERSION}_1240K_public.anno")
ANNO_FILE_HO = pathlib.Path(f"AADR/{AADR_VERSION}_HO/aadr_{AADR_VERSION}_HO_public.anno")
IND_FILE_1240K = pathlib.Path(f"AADR/{AADR_VERSION}_1240K/aadr_{AADR_VERSION}_1240K_public.ind")
...
def keepOnlyCapture(annoFrame):
    ...
    assert (not np.any([((".SG" in x) or (".DG" in x)) for x in annoFrame[captureMask][GENETIC_ID]]))
    # this should be all the other ones
    shotgunMask = np.isin(annoFrame['Data_source'], ['Shotgun', 'Shotgun.diploid'])
    # check whether names are somewhat consitent with labeling
    assert (np.all([((".SG" in x) or (".DG" in x)) for x in annoFrame[shotgunMask][GENETIC_ID]]))
...
def createMockIndFile(inputHoIndFile, outputMockFile, focalFrame):
    ...
    aadrHoIndFrame = pd.read_csv(inputHoIndFile, delim_whitespace=True, header=None,
    ...
    assert (set(focalFrame['Data_source']).issubset({'Shotgun', 'Shotgun.diploid', '1240K', '1240k'}))
    ...
    assert (np.sum(captureMask) == len(captureIds)), (np.sum(captureMask), len(captureIds))
...
def main():
    ...
    indFrame = pd.read_csv(IND_FILE_1240K, delim_whitespace=True, header=None, names=['ID', 'sex', 'location'])
    indFrameHO = pd.read_csv(IND_FILE_HO, delim_whitespace=True, header=None, names=['ID', 'sex', 'location'])
