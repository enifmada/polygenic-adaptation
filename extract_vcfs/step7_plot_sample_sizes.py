...
for geno in GENOTYPING:
    SAMPLE_SIZES_TABLES[geno] = pathlib.Path (f"{EXTRACTED_PREFIX}_{geno}_sample_sizes.table")
    SAMPLE_SIZES_PDFS[geno] = pathlib.Path (f"{EXTRACTED_PREFIX}_{geno}_sample_sizes.pdf")
...
def plotSampleSizes (annoFrame, sampleSizeTable, sampleSizePdf):
    ...
    YEARS_PER_GEN = 28
    sampleCounter = collections.Counter ((np.array(annoFrame['Date_mean'])/YEARS_PER_GEN).astype(int))
    # plot it
    ...
    for (k,v) in sampleCounter.items():
        ...
        # needs pandas 2.0
        sampleSizes = pd.concat([sampleSizes, pd.DataFrame([newRow])], ignore_index=True)
    # plt.yscale ("log")
