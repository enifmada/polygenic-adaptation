...
for geno in GENOTYPING:
    SAMPLE_SIZES_TABLES[geno] = pathlib.Path(
        f"{EXTRACTED_PREFIX}_{geno}_sample_sizes.table"
    )
    SAMPLE_SIZES_PDFS[geno] = pathlib.Path(
        f"{EXTRACTED_PREFIX}_{geno}_sample_sizes.pdf"
    )
...
def plotSampleSizes(annoFrame, sampleSizeTable, sampleSizePdf):
    ...
    YEARS_PER_GEN = 28
    sampleCounter = collections.Counter(
        (np.array(annoFrame["Date_mean"]) / YEARS_PER_GEN).astype(int)
    )
    # plot it
    sampleSizes = pd.DataFrame(columns=["time", "sample_size"])
    for k, v in sampleCounter.items():
        ...
        sampleSizes = pd.concat(
            [sampleSizes, pd.DataFrame([newRow])], ignore_index=True
        )
    # plt.yscale ("log")
    plt.xlim(
        -max(list(sampleCounter.keys())) * YEARS_PER_GEN + YEARS_PER_GEN,
        3 * YEARS_PER_GEN,
    )
