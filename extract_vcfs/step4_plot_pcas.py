...
def plotPCA (pcs, evals, categories, pca_plot_file, selectMask=None, plotCentroid=False,axtext=""):
...
def allPcaPlots():
    ...
    short_pubs = np.array ([x.split()[0][0] + x.split()[0][-2:] for x in anno['Publication']])
    ...
    for reference in ['broad', 'europe', 'gbr_ceu']:
        for to_shrink in ['shrinkage', 'noshrinkage']:
            for genotyping in ['capture_only', 'capture_SG']:
                ...
                eval_file = pathlib.Path (f"{EXTRACTED_PREFIX}_{genotyping}_{reference}_pca_{to_shrink}.eval")
                evec_file = pathlib.Path (f"{EXTRACTED_PREFIX}_{genotyping}_{reference}_pca_{to_shrink}.evec")
                ...
                leveraged_pubs = np.array(['' if (x not in tmp_anno_ids) else (short_pubs[tmp_anno_ids.index(x)]) for x in ids])
                ...
                pca_plot_file = pathlib.Path (f"{EXTRACTED_PREFIX}_{genotyping}_{reference}_pca_{to_shrink}.pdf")
                plotCentroid = reference != 'broad'
                plotPCA (pcs, evals, categories, pca_plot_file, plotCentroid=plotCentroid, axtext=axtext)
