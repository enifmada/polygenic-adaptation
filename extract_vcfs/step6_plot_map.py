...
ALL_GENOTYPES_SET = {val for dic in GENOTYPING.values() for val in dic}
MAP_FILES = {}
for geno in GENOTYPING:
    MAP_FILES[geno] = pathlib.Path(f"{EXTRACTED_PREFIX}_{geno}_map.pdf")
...
def plotMap(annoFrame, mapFile):
    ...
    print(np.sum(~latLongMask))
    print("Individuals without coordinates:")
    print(
        annoFrame[~latLongMask][["Genetic_ID", "Date_mean", "Locality", "Publication"]]
    )

    plotIndividuals = annoFrame.loc[latLongMask]

    age_years = np.array(plotIndividuals["Date_mean"]).astype(float)
    longs = np.array(plotIndividuals["Long."]).astype(float)
    lats = np.array(plotIndividuals["Lat."]).astype(float)
    # use master id because of potential duplicates
    np.array(plotIndividuals["Master_ID"])
    ...
    # needs cartopy 0.21
    im = ax.scatter(
        longs, lats, c=age_years, transform=cartopy.crs.PlateCarree(), s=30, alpha=0.75
    )
    ...
    fig.canvas.draw()
    plt.colorbar(im, label="Years before present", shrink=0.75)
