...
def main():
    ...
    large_xspace = np.linspace(large_density.ppf(.01), large_density.ppf(.99), 1000)
    small_xspace = np.linspace(small_density.ppf(.01), small_density.ppf(.99), 200)
    ...
    axs.fill_between(plot_space_small, small_density, color="gray", alpha=.5)
    fig.suptitle(f"{plot_title} density plot")
    fig.savefig(smk.output[0], format="pdf", bbox_inches="tight")

    fig, axs = plt.subplots(1,1, figsize=(10,5), layout="constrained")
    axs.fill_between(plot_space_large, np.log(large_density), color="gray", alpha=.5)
