...
def compute_avg_variance(slim_array, betas_array, stderr_array=None):
    if stderr_array is not None:
        init_variance = 2 * np.sum(
            slim_array[3:, 0] * (1 - slim_array[3:, 0]) * (betas_array**2 + stderr_array**2)
        )
        final_variance = 2 * np.sum(
            slim_array[3:, -1] * (1 - slim_array[3:, -1]) * (betas_array**2 + stderr_array**2)
        )
    else:
        init_variance = 2 * np.sum(
...
def main():
    ...
    parser.add_argument("-dz", type=float, default=0., help="distance to optimum (directional only)")
    parser.add_argument("-h2", default=1., type=float, help="heritability")
    ...
    for input_i in tqdm(range(loop_len)):
        ...
        if smk.gwas:
            ...
            fake_noisy_betas = rng.normal(true_beta*(1-temp_d_over_vg), np.mean(stderrs_array), size=betas_array.shape[0])
        ...
        for b_i, grid_betas in enumerate(betas_list):
            ...
            actual_berrs = np.abs(BETA_EXPONENT * errors_list[b_i][usable_locs] * grid_betas[usable_locs] ** (BETA_EXPONENT - 1))
            ...
            if np.all(np.isclose(actual_berrs, 0)):
                ...
                scatterplot = axs.scatter(actual_betas, actual_s, s=actual_serrs / np.min(actual_serrs),
                                          c=actual_berrs)
                fig.colorbar(scatterplot, ax=axs)
            axs.set_title(f"min - max s err: {np.min(actual_serrs):.4f} - {np.max(actual_serrs):.4f}")
            x_space = np.linspace(np.min(actual_betas) * .95, np.max(actual_betas) * 1.05, 500)
            axs.plot(x_space, -theory_list[b_i] * SCALING_FACTOR * x_space,
    ...
    for b_i in range(str_ests.shape[0]):
        ...
        if smk.gwas:
            ...
            if smk.mode == "directional":
                ...
                axs3.plot(x_data, np.sqrt(1/(-str_ests[b_i]) - sigma_sqs * (1-d_over_vg) - V_E)/x_data -1, ".", label=str_combo_labels[b_i])
            else:
                axs3.plot(x_data, np.sqrt((1-d_over_vg)**2/(-str_ests[b_i]) - sigma_sqs * (1-d_over_vg) - V_E)/x_data-1, ".", label=str_combo_labels[b_i])
