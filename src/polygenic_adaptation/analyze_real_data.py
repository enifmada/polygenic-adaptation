...
def main():
    ...
    for grid_i in tqdm(range(num_inputs//2)):
        ...
        assert (
            trait_name
            in Path(smk.input[grid_i + num_inputs // 2]).name
        )
    ...
    axs1.plot(np.arange(len(direc_unif_estimates))+1, direc_unif_estimates[direc_argsort], "b*")
    axs1.set_xticks(np.arange(len(direc_unif_estimates))+1, labels=trait_names[direc_argsort], rotation=90)
    ...
    axs2.set_xticks(np.arange(len(stab_unif_estimates))+1, labels=trait_names[stab_argsort], rotation=90)
