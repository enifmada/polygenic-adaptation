...
def subset_pheno(input_file, output_file, adna_df):
    ...
    pheno_list = list(
        zip(pheno_file["chr"].values, pheno_file["loc"].values, pheno_file["ref"].values, pheno_file["alt"].values, strict=False))
    adna_list = list(
        zip(adna_df["all_chrom"].values, adna_df["all_loc_per_chrom"].values, adna_df["all_ref_allele"].values,
            adna_df["all_alt_allele"].values, strict=False))
    bool_mask = pd.Series(pheno_list).isin(adna_list)
    pheno_file_subset = pheno_file[bool_mask].reset_index(drop=True)
    red_pheno_list = list(
        zip(pheno_file_subset["chr"].values, pheno_file_subset["loc"].values, pheno_file_subset["ref"].values,
            pheno_file_subset["alt"].values, strict=False))
...
def main():
    ...
    adna_df = pd.DataFrame({k: all_adna_snp_data[k] for k in ["all_loc_per_chrom", "all_chrom", "all_rsid", "all_ref_allele", "all_alt_allele"]})
