...
def main():
    ...
    for frac_err in [0.0, 0.1, 0.2, 0.5]:
        ...
        for input_i in range(len(smk.input)):
            ...
            for corr_factor, gb_text in zip([(1+frac_err**2)*(cur_bulmer**2)-2*frac_err**2], correction_types, strict=False):

                expanded_direc_x = np.linspace(raw_grid[0] / (2 * max_signed_beta),raw_grid[-1] / (2 * max_signed_beta),10000)
                direc_s2l_raw_1 = raw_grid[1] / (2 * max_signed_beta)
                direc_s2l_raw_2 = raw_grid[-2] / (2 * max_signed_beta)

                expanded_stab_x = np.linspace(raw_grid[0] / (max_signed_beta**2 * corr_factor),raw_grid[-1] / (max_signed_beta**2 * corr_factor),10000,)
                ...
                if frac_err <= 0:
                    ...
                    axs[0].plot(raw_grid/(2*max_signed_beta), np.zeros_like(raw_grid)+np.max(all_dll_unif_ests), "r.")
                    axs[1].plot(expanded_stab_x, summed_unif_slls / all_dll_unif_ests.shape[0], color="k", lw=2,)
                    axs[1].plot(raw_grid/(max_signed_beta**2 * corr_factor/2), np.zeros_like(raw_grid)+np.max(all_sll_unif_ests), "r.")
                    ...
                    fig.savefig(output_path.with_stem(output_path.stem+err_str), format="pdf", bbox_inches="tight",)
                    plt.close(fig)
