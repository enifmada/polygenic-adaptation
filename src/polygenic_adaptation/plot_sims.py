...
def main():
    ...
    parser.add_argument(
        "--output_names", nargs="*", help="output names for each plot type - full paths will be inferred"
    )
    parser.add_argument(
        "--group_by_reg_type",
        action="store_true",
        help="make one plot for each regression type, with one box per label. otherwise make one set of two boxplots per label",
    )
    ...
    V_E = (
        dframe["sigma_sqs"] * (1 - dframe["x_vars"]) / dframe["x_vars"]
        if smk.vary == "h2"
        else dframe["sigma_sqs"] * (1 - dframe["h2s"]) / dframe["h2s"]
    )
    ...
    if smk.group_by_reg_type:
        ...
        for regtype in ["true", "wls", "odr"]:
            ...
            if mode == "directional":
                ...
                w_plot_y = (
                    np.sqrt((1 - d_over_vg) ** 2 / (S_est) - dframe["sigma_sqs"] * (1 - d_over_vg) - V_E)
                    / dframe["x_vars"]
                    - 1
                )
            ...
            if pts_per_param_val > 5:
                ...
                for labeli in smk.labels:
                    ...
                    axs_Srelerr.plot(
                        dframe_sub["x_vars"], (S_est / str_theory - 1)[dframe["labels"] == labeli], ".", label=labeli
                    )
                    axs_wrelerr.plot(dframe_sub["x_vars"], w_plot_y[dframe["labels"] == labeli], ".", label=labeli)
                    axs_zsc_sig.plot(
                        dframe_sub["x_vars"], (S_est / S_err)[dframe["labels"] == labeli], ".", label=labeli
                    )
                    axs_zsc_err.plot(
                        dframe_sub["x_vars"],
                        ((S_est - str_theory) / S_err)[dframe["labels"] == labeli],
                        ".",
                        label=labeli,
                    )
            box_df = pdDataFrame(
                data=zip(box_x_data, box_y_data, box_labels, strict=False), columns=[x_label, y_label, "Label"]
            )
            ...
            fig_box.savefig(
                save_path.parent / (smk.output_names[0] + f"_{regtype}.pdf"), format="pdf", bbox_inches="tight"
            )
            plt.close(fig_box)
            if pts_per_param_val > 0:
                for ydata, ylabel, axs, fig, num in zip(
                    ...
                    [
                        r"$\frac{\hat{S}-S}{S}$",
                        r"$\frac{\hat{\omega}-\omega}{\omega}$",
                        "Z-score",
                        r"$\frac{\hat{S} - S}{\hat{se}_S}$",
                    ],
                    [axs_Srelerr, axs_wrelerr, axs_zsc_sig, axs_zsc_err],
                    [fig_Srelerr, fig_wrelerr, fig_zsc_sig, fig_zsc_err],
                    [1, 2, 3, 4],
                    strict=False,
                ):
                    boxplot_df = pdDataFrame(
                        data=zip(nonbox_x_data, ydata, nonbox_labels, strict=False),
                    ...
                    axs.legend()
                    fig.savefig(
                        save_path.parent / (smk.output_names[num] + f"_{regtype}.pdf"),
                        format="pdf",
                        bbox_inches="tight",
                    )
                    plt.close(fig)
        ...
        for ip_i in range(len(smk.input_parquets)):
            ...
            for str_theory, S_est, S_err, box_x, box_y, box_l, box_ax, label, box_tl in zip(
                [
                    str_theory_bulmer[dframe_mask],
                    str_theory_semibulmer[dframe_mask],
                    str_theory_semibulmer[dframe_mask],
                ],
                ...
                [theory_str_bm, theory_str_sbm, theory_str_sbm],
                strict=False,
            ):
                # if "WLS" in label:
                #     continue
                ...
                if mode == "directional":
                    ...
                    w_plot_y = (
                        np.sqrt(1 / (S_est) - dframe_subset["sigma_sqs"] * (1 - d_over_vg) - V_E)
                        / dframe_subset["x_vars"]
                        - 1
                    )
                else:
                    w_plot_y = (
                        np.sqrt((1 - d_over_vg) ** 2 / (S_est) - dframe_subset["sigma_sqs"] * (1 - d_over_vg) - V_E)
                        / dframe_subset["x_vars"]
                        - 1
                    )
            ...
            box_gt_df = pdDataFrame(
                data=zip(box_gt_x_data, box_gt_y_data, box_gt_labels, strict=False),
                columns=[x_label, y_label, "Data type"],
            )
            box_gwas_df = pdDataFrame(
                data=zip(box_gwas_x_data, box_gwas_y_data, box_gwas_labels, strict=False),
                columns=[x_label, y_label, "Data type"],
            )
            # if smk.mode == "directional":
            #     box_gwas_df = pdconcat([box_gt_df, box_gwas_df])
            for df, ax, fig, num, _subfig_l in zip(
                [box_gt_df, box_gwas_df],
