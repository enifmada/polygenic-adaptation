...
def main():
    ...
    for input_i in tqdm(range(loop_len)):
        ...
        if np.any(~np.isfinite(s_unif_vals)):
            ...
            if bad_1h_locs[0].shape[0]>0:
                endbad_1h_locs = np.concatenate((np.where(np.diff(bad_1h_locs[0])>0)[0], [bad_1h_locs[0].shape[0]-1]))
                for ebi in np.arange(endbad_1h_locs.shape[0]):
                    s_unif_vals[bad_1h_locs[0][endbad_1h_locs[ebi]], :bad_1h_locs[1][endbad_1h_locs[ebi]] + 1] = s_unif_vals[bad_1h_locs[0][endbad_1h_locs[ebi]], bad_1h_locs[1][endbad_1h_locs[ebi]] + 1]
            ...
            if bad_2h_locs[0].shape[0]>0:
                ...
                for ebi in np.arange(endbad_2h_locs.shape[0]):
                    s_unif_vals[bad_2h_locs[0][endbad_2h_locs[ebi]], bad_2h_locs[1][endbad_2h_locs[ebi]]+raw_grid.shape[0]//2:] = s_unif_vals[bad_2h_locs[0][endbad_2h_locs[ebi]], bad_2h_locs[1][endbad_2h_locs[ebi]] + raw_grid.shape[0]//2-1]
        ...
        usable_b_locs = (stderrs_array != 0) & (np.abs(gwas_betas_array) < top_tenth)#& (np.abs(gwas_betas_array) > bottom_tenth)#
        ...
        sc_axs_gt.scatter(actual_betas_gt, actual_s_gt, color=colorlist[0], s=7/(actual_serrs_gt/ np.min(actual_serrs_gt)))
        #sc_axs_gt.set_title(f"min - max s err: {np.min(actual_serrs_gt):.4f} - {np.max(actual_serrs_gt):.4f}")
        gwas_scatterplot = sc_axs_gwas.scatter(actual_betas_gwas, actual_s_gwas, s=actual_serrs_gwas / np.min(actual_serrs_gwas), c=actual_berrs_gwas)
        ...
        if np.sum(usable_locs_gwas) > 0:
            ...
            odr_data = scipy.odr.RealData(gwas_betas_array[usable_locs_gwas], actual_s_gwas, sx=stderrs_array[usable_locs_gwas], sy=actual_serrs_gwas)
            ...
            m_wls_gwas = ((betas_wls_gwas.T @ wls_weights_gwas @ actual_s_gwas) / (betas_wls_gwas.T @ wls_weights_gwas @ betas_wls_gwas))[0][0]
            ...
            sc_axs_gwas.plot(x_space_gwas, m_wls_gwas * x_space_gwas,
                             label=rf"$s_\ell = {m_wls_gwas:.4f}\beta_\ell{'^' + str(BETA_EXPONENT) if BETA_EXPONENT == 2 else ''}$ (WLS)")
            sc_axs_gwas.plot(x_space_gwas, m_odr * x_space_gwas,
            ...
            sc_axs_gwas.fill_between(x_space_gwas, m_odr * x_space_gwas - 1.96 * err_odr,
                                     m_odr * x_space_gwas + 1.96 * err_odr, alpha=.4, label="ODR err")
            sc_axs_gwas.plot(x_space_gwas, -temp_theory_semibulmer * SCALING_FACTOR * x_space_gwas,
        ...
        fname = ("surfaces_cond/" + temp_path.name.rpartition("_grid")[0] + f"_regression_gwasbetas{'_uc' if 'uncon' in temp_path.name else ''}.pdf")
        ...
        if np.sum(usable_locs_gt) > 0:
            ...
            m_wls_gt = ((betas_wls_gt.T @ wls_weights_gt @ actual_s_gt) / (betas_wls_gt.T @ wls_weights_gt @ betas_wls_gt))[0][0]
            ...
            sc_axs_gt.plot(x_space_gt,m_wls_gt * x_space_gt,
                label=rf"$s_\ell = {m_wls_gt:.2f}\beta_\ell{'^' + str(BETA_EXPONENT) if BETA_EXPONENT == 2 else ''}$", color=colorlist[1])
        ...
        temp_path = Path(smk.input[input_i])
        temp_parent = temp_path.parent.parent
        fname = ("surfaces_cond/"+ temp_path.name.rpartition("_grid")[0]+ f"_regression_truebetas{'_uc' if 'uncon' in temp_path.name else ''}.pdf")
    ...
    labels = ["files", "modes", "dzs", "h2s", omega_dict_str, "sigma_sqs", "x_vars", "S_ests_true", "S_errs_true", "S_ests_wls", "S_errs_wls", "S_ests_odr", "S_errs_odr"]
    ...
    datas = [smk.input[:-1], modes, dzs, h2s, omegas, sigma_sqs, x_vars, S_ests_true, S_errs_true, S_ests_wls, S_errs_wls, S_ests_odr, S_errs_odr]
