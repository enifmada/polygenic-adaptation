...
def main():
    ...
    for s_i, S_str in enumerate(["direc", "stab"]):
        ...
        axs.plot(np.arange(everything_df.shape[0]) + .85, S_est_odr[S_argsort], "bo")
        axs.plot(np.arange(everything_df.shape[0]) + 1.15, S_est_wls[S_argsort], "go")
        axs.errorbar(np.arange(everything_df.shape[0])+0.8, S_est_odr[S_argsort], yerr=S_err_odr[S_argsort], fmt="none", ecolor="k", label="ODR")
        ...
        axs.plot((np.arange(everything_df.shape[0])+0.8)[sig_ps_odr],
                  (S_est_odr[S_argsort] + S_err_odr[S_argsort] + 0.01*yext)[sig_ps_odr], "k*")
