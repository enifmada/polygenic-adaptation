...
def find_peaks_greedy(ps, xs, chrs, min_height, window_size):
    ...
    while np.max(poss_peaks) > min_height:
        ...
        surr_mask = (poss_chrs == poss_chrs[max_idx]) & (poss_xs < poss_xs[max_idx] + window_size) & (poss_xs > poss_xs[max_idx] - window_size)
...
def main():
    ...
    paired_snps_alleles = zip(adna_data["all_rsid"], adna_data["all_ref_allele"], adna_data["all_alt_allele"], adna_data["all_chrom"], adna_data["all_loc_per_chrom"], strict=False)
    filtered_sumstats_array = sumstats_array[sumstats_array[["SNP", "REF", "A2", "CHR", "POS"]].apply(tuple, axis=1).isin(paired_snps_alleles)].reset_index()
    ...
    if smk.mode[0]=="LDetect":
        ...
        for block in ld_blocks.itertuples(index=False):
            filtered_subset = filtered_sumstats_array[(filtered_sumstats_array["CHR"]==block[0])&(filtered_sumstats_array["POS"]>=block[1])&(filtered_sumstats_array["POS"]<block[2])&(filtered_sumstats_array["ash_p"]<p_bh)]
            if filtered_subset.shape[0]:
                idxs.append(filtered_subset["ash_p"].idxmin())
    else:
        idxs = find_peaks_greedy(-np.log10(filtered_sumstats_array["ash_p"].to_numpy()), filtered_sumstats_array["POS"].to_numpy(), filtered_sumstats_array["CHR"].to_numpy(), float(smk.height[0]), int(smk.width[0]))
    ...
    assert np.intersect1d(adna_data["all_rsid"][pruned_adna_idxs], pruned_sumstats_snps).shape[0] == pruned_adna_idxs.shape[0]
    ...
    np.savetxt(smk.output[2], adna_csv[pruned_adna_idxs, :], delimiter="\t", fmt="%d",
