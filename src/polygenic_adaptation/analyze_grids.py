...
def main():
    ...
    for grid_i in tqdm(range((num_inputs - 1) // 2)):
        ...
        for loc in range(dll_vals.shape[0]):
            ...
            if betas[loc] >= 0:
                dll_spline = PchipInterpolator(sdz_est_grid, dll_vals[loc, :])
            else:
                dll_spline = PchipInterpolator(sdz_est_grid[::-1], dll_vals[loc, ::-1])
        ...
        fig.savefig(
            Path(smk.output[1]).parent / f"{grid_i}_all_pchip_ests.pdf",
            format="pdf",
            bbox_inches="tight",
        )
        plt.close(fig)
