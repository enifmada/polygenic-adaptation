...
def main():
    ...
    parser.add_argument("--no_below_zero", action="store_true", help="use this flag to set values below 0 to 0 because they are not real")
    ...
    for s_i, S_str in enumerate(["direc", "stab"]):
        ...
        dotcolor = colorlist[0] if S_str == "direc" else colorlist[-1]
        ...
        np.where(ps_wls < corr_p_val)[0]
        np.where(ps_jko < corr_p_val)[0]
        np.where(ps_jkw < corr_p_val)[0]
        fig, axs = plt.subplots(1, 1, figsize=(6.1, 6.1), layout="constrained")
        axs.plot(np.arange(everything_df.shape[0]) +1, S_est_odr[S_argsort], "o", color=dotcolor)
        #axs.plot(np.arange(everything_df.shape[0]) + 1.15, S_est_wls[S_argsort], "go")
        axs.errorbar(np.arange(everything_df.shape[0])+1, S_est_odr[S_argsort], yerr=S_err_odr[S_argsort], fmt="none", capsize=2, ecolor="k", label="ODR")
        ...
        #axs.errorbar(np.arange(everything_df.shape[0]) + 1.1, S_est_wls[S_argsort], yerr=S_err_wls[S_argsort], fmt="none", ecolor="m",label="WLS")
        ...
        axs.set_xticks(
            ...
            labels=everything_df["trait_desc"].to_numpy()[S_argsort] if "trait_desc" in everything_df.columns else everything_df["trait_name"].to_numpy()[S_argsort],
            rotation=90,
        )
        axs.plot((np.arange(everything_df.shape[0])+1)[sig_ps_odr],
                  (S_est_odr[S_argsort] + S_err_odr[S_argsort] + 0.012*yext)[sig_ps_odr], "r*")
        #axs.plot((np.arange(everything_df.shape[0]) + 0.9)[sig_ps_jko],
                 #(S_est_odr[S_argsort] + S_err_jko[S_argsort] + 0.01 * yext)[sig_ps_jko], "r*")
        # axs.plot((np.arange(everything_df.shape[0])+1.1)[sig_ps_wls],
        #           (S_est_wls[S_argsort] + S_err_wls[S_argsort] + 0.01*yext)[sig_ps_wls], "m*")
        ...
        axs2.plot(np.arange(everything_df.shape[0]) + .95, (S_est_odr/S_err_odr)[z_argsort], "bo", label="ODR")
        ...
        axs2.set_xticks(
            np.arange(everything_df.shape[0]) + 1,
            labels=everything_df["trait_desc"].to_numpy()[z_argsort] if "trait_desc" in everything_df.columns else everything_df["trait_name"].to_numpy()[z_argsort],
            rotation=90,
        )
        axs2.legend()
    ...
    fig3, axs3 = plt.subplots(1,1,figsize=(everything_df.shape[0]/5,everything_df.shape[0]/5), layout="constrained")
    ...
    axs3.set_xticks(
        np.arange(everything_df.shape[0]) + 1,
        labels=everything_df["trait_desc"].to_numpy()[stab_w_argsort] if "trait_desc" in everything_df.columns else everything_df["trait_name"].to_numpy()[stab_w_argsort],
        rotation=90,
    )
    axs3.legend()
