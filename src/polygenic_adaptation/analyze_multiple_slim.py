...
def main():
    ...
    parser.add_argument("--gwas", action="store_true", help="flag for if the betas are taken from a gwas vs ground truth")
    ...
    for input_i in range(loop_len):
        ...
        expanded_stab_x = np.linspace(raw_grid[0] / (max_signed_beta**2/2), raw_grid[-1] / (max_signed_beta**2/2),10000,)
        ...
        axs[1].plot(raw_grid/(max_signed_beta**2/2), np.zeros_like(raw_grid)+np.max(all_sll_unif_ests), "r.")
        ...
        fig.savefig(smk.output[input_i], format="pdf", bbox_inches="tight",)
        plt.close(fig)
