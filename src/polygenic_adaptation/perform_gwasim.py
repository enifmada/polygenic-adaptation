...
def main():
    ...
    parser.add_argument("--source", help="whether the simulation was done by slim or polyslim")
    smk = parser.parse_args()

    assert smk.source in ["polysim", "slim"]
    ...
    if smk.source == "slim":
        ...
        phenos = np.load(smk.phenotypes)["arr_0"]
        genos = np.load(smk.genotypes)["arr_0"].astype(int)
        genotype_by_site = genos[:, ::2] + genos[:, 1::2]
        max_count = genos.shape[1]
    count_by_site = np.sum(genotype_by_site, axis=-1)
    good_idxs = np.where((count_by_site > 0) & (count_by_site < max_count))[0]
